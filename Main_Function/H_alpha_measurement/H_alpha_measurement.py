...
#         WkmsEW=c*(1.0-rEW)/(1.0+rEW)
        ...
        return W_ha,W_haerr,Wkms,Wkmserr,'e',radial_velocity
    else:
        return W_ha,W_haerr,np.float64('nan'),np.float64('nan'),'a',radial_velocity
...
for i in scan_file(gndir):
    try:
        ...
        EWHa,EWHaerr,WHakms,WHakmserr,eora,v=Ha(file)
...
label=['starlist','_10_percent','RV_ref','Eq_width','e_or_a']
total=[starlist,_10_percent,RV_ref,Eq_width,e_or_a]
...
dirr = temp_dir + '\\'
filename = 'test_file'
#print(len(starlist), len(_10_percent), len(RV_ref), len(Eq_width), len(Halfmax_width), len(e_or_a))

# Call the function
writedat(dirr, filename, total, label)
print(errorlist)
