...
#     #print(clean_filenames, clean_elements)

#     # 创建一个映射元素到文件名的字典
#     element_to_filename = {element: filename for filename in clean_filenames for element in clean_elements if filename.endswith(element+'.fits')}
#     # 按照元素列表的顺序对文件名进行排序
...
def sort(elements, filenames):
    # 清理元素和文件名中的空格
    clean_elements = [element.strip() for element in elements]
    clean_filenames = [filename.strip() for filename in filenames]

    # 创建一个映射元素到文件名的字典
    element_to_filename = {}
    for filename in clean_filenames:
        for element in clean_elements:
            if filename.endswith(element + '.png'):
                element_to_filename[element] = filename
                break

    # 按照元素列表的顺序对文件名进行排序
    sorted_filenames = [element_to_filename[element] for element in clean_elements if element in element_to_filename]
...
output_path=r'C:\Users\ZY\Documents\github\233boy\Dr.-Yep-2024-summer-research\norm_final0'
...
#     print(i)
#     w,f=openfits(sorted_filenames[i])
#     name=f'{dic[i][0]}:{dic[i][1]}'
...
for i in sorted_image_paths:
    print(i)
combine_images_vertical(sorted_image_paths, output_path) 
